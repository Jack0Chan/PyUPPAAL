...
class Monitors:
    ...
    def observer_suffix_monitor(name: str, suffix_sequence: List[str], sigma_o: List[str], sigma_un: List[str], init_ref: int) -> Template:
        ...
        Args:
            ...
            init_ref (int): The reference ID for the first location in the monitor.
    ...
    def obs_after_fault_monitor(name: str, suffix_sequence: List[str], sigma_o: List[str], sigma_un: List[str], fault: str, init_ref: int) -> Template:
        ...
        Args:
            ...
            sigma_o (List[str]):  observable events
            sigma_un (List[str]):  unobservable events
            fault (str):  fault name 
            init_ref (int): init location id
    ...
    def input_after_fault_monitor(
        ...
        Args:
            identified_faults (List[str]): 被identify出的faults
            protecting_events (List[str]): 保护措施
            sigma_control (List[str]): 控制event集合
            control_length (int): 控制长度限制
    ...
    @staticmethod
    def input_template(name: str, signals: List[str] | List[Tuple[str, str, str]], init_id: int) -> Template:
        """ input_template
        ...
        Args:
            ...
            init_id (int):  the start id of the monitor
    ...
    def observer_template(name: str, signals: List[Tuple[str, str, str]], observe_action: List[str],
                         init_id: int, strict: bool = False, allpattern: bool = False) -> Template:
        """ observer_template
        ...
        for i, signal_i in enumerate(signals):
            ...
            location = Location(location_id=init_id + i, location_pos=(300 * i, 200), invariant=inv)
            locations.append(location)
        # 需要多一个尾巴location
        location = Location(location_id=init_id + len(signals), location_pos=(300 * len(signals), 200), name='pass')
        locations.append(location)
        ...
        for i, signal_i in enumerate(signals):
            ...
            edge = Edge(source_location_id=init_id + i, target_location_id=init_id + i + 1,
            ...
            for i, signal_i in enumerate(signals):
                ...
                for j, action_j in enumerate(pruned_observation_action):
                    ...
                    if allpattern and (pruned_observation_action[j] == signals[i][0]):
                        continue
                    if i == 0:
                        transition = Edge(source_location_id=init_id + i, target_location_id=fail_location_id,
        ...
        monitor_elem = Template(name=name, locations=locations,
                                init_ref=init_id, edges=edges, declaration=declaration)
        return monitor_elem
